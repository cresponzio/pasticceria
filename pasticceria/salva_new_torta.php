        ...
        <?php  
              if( isset($_SESSION["id"]) && $_SESSION["id"] && $_POST  ) {    
                ...
                if( $uploadOk && $db = some_query::connect() ){     
                    ...
                    if( some_query::add_cake( $db, $nome, $prezzo, $target_file ) ){

                      $id=some_query::lastID($db);
                     
                      for( $i=0; $i<count($v_ingr); $i++ ){
                          if( $v_qnt[$i] )
                              some_query::add_ingredient( $db, $id, $v_ingr[$i], $v_qnt[$i], $v_misura[$i] );
                      }
